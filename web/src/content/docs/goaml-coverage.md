...
1. **Real-time transaction-scoring API** — `POST /transactions/score` returns a per-transaction decision (approve / review / hold / reject) with explainable reasons in under 500ms. Bank core-banking systems integrate via a single HTTP call. Inline sanctions screening on the from / to parties when the payload carries names. Feedback endpoint captures legitimate / fraud labels for continuous improvement.
2. **Sanctions / PEP / adverse-media screening** — `POST /screening/entity` against a 27,000+-record watchlist pool aggregating OFAC, UN, UK FCDO, and BB Domestic; daily refresh Beat task; ComplyAdvantage adapter for paid adverse-media and PEP feeds.
3. **KYC / CDD onboarding** — `/customers` route family runs sanctions screening inline on the primary candidate plus every beneficial owner during onboarding. Periodic re-screening Beat task at 03:00 BDT picks up newly-listed parties against the existing customer book; emits Alerts + Cases on new hits.
4. **Agentic AI investigations** — bounded multi-step agent at `POST /agents/investigate` (8-hop + 60s wall-clock caps, 6-tool whitelist) walks an entity through neighbours / recent alerts / recent STRs / sanctions / narrative, returns hypothesis + evidence trail + suggested actions. "Promote to STR draft" stows the result for the analyst.
5. **AI-powered entity extraction** from STR narratives — automatic subject resolution on submit.
6. **AI-drafted STR narratives** — analysts describe facts, Kestrel drafts the narrative; "Draft STR from alert" action on every alert.
7. **AI alert explanations** — every alert opens with an AI-generated "why this fired" panel; no click required.
8. **AI outcome logging + sovereign-AI roadmap** — every AI invocation writes to `ai_outcome_log` with redacted prompt, structured output, latency, and token counts. Analyst corrections build the corpus for a Bangladesh-specific fine-tune on Llama / Qwen via the LoRA training scaffold; per-task confidence routing and Beat-driven rollback are wired so sovereign traffic can be flipped on per-task at any rollout percentage.
9. **Automatic cross-bank entity resolution** — `pg_trgm` fuzzy matching + `same_owner` graph edges emitted on every STR submit and scan run; shared entity pool with regulator-wide visibility. Persona-aware anonymisation hides peer-bank identity from CAMLCO views (see the cross-bank intelligence whitepaper).
10. **Network graphs on every entity** — automatic React Flow canvas, risk-weighted edges, suspicious-path detection.
11. **8 pre-built detection rules** with continuous scoring — rapid cashout, fan-in / fan-out burst, dormant spike, layering, proximity to flagged, structuring, first-time high value — plus admin-configurable JSON-DSL custom match definitions.
12. **Compliance scorecard** — ranks banks against each other on submission timeliness, alert conversion, and peer coverage.
13. **Cloud + on-prem flexibility** — same engine code runs on the hosted SaaS or inside a customer data centre via the on-prem Docker image set; air-gapped AI routing skips OpenAI / Anthropic entirely when `KESTREL_DEPLOYMENT_MODE=onprem`; offline watchlist archive import for air-gapped tenants.
14. **Modern web interface** — accessible from any browser, no desktop install, no VPN-only access.
15. **Production observability + 99.5% / 99.9% SLA** — structured JSON logs, per-request tracing via `X-Request-ID`, standardised error envelopes, incident runbook, readiness probe at `GET /ready` covering auth + database + Redis + storage + Celery worker + AI providers, public status page driven by 5-minute uptime ledger.
16. **BDT-native, Bangla-ready** — built for Bangladesh specifically; reference tables seeded with scheduled banks, MFS providers, recipient agencies, and BFIU-relevant country / currency subsets.
17. **Self-serve bank pilot in 10 minutes** — `/signup/bank` provisions the tenant, fires the demo seed via Beat, and hands the new CAMLCO a populated workspace within ten minutes of magic-link click.
18. **goAML round-trip** — the XML import + export pair means banks can keep their existing pipelines, and BFIU can hand off to peer FIUs or legacy systems without rewriting anything.
...
| Public status | `GET /status/summary`, `/status/incidents`, `/status/plans` | Public uptime + recent incidents + tier overview. |
...
| Real-time scoring | `POST /transactions/score` (+ feedback, recent stream, metrics) | Per-transaction decisioning under 500ms with explainable reasons. |
| Sanctions / PEP / adverse-media screening | `POST /screening/entity`, `/screening/adverse-media`, `GET /screening/entries`, `POST /screening/entries` | Fuzzy match against 27,000+ records across OFAC / UN / UK / BB Domestic / PEP. |
| KYC / CDD | `/customers` (onboard, list, detail, update, review, rescreen) | Inline sanctions screening on primary + beneficial owners. |
| Agentic investigations | `POST /agents/investigate`, `/agents/investigations` | Bounded multi-step AI agent with evidence trail. |
| Intelligence | `/intelligence/entities` (search + new subject), `/intelligence/matches`, `/intelligence/typologies`, `/intelligence/cross-bank/{summary,matches,heatmap,top-entities}` | Cross-bank views are persona-aware. |
| Saved queries | `/saved-queries` | |
| Diagrams | `/diagrams` | |
| Match definitions | `/match-definitions` (list, detail, execute) | |
| Reference tables | `/reference-tables` (any read, regulator write) | |
| Admin | `/admin/summary`, `/admin/settings`, `/admin/team`, `/admin/rules`, `/admin/api-keys`, `/admin/statistics`, `/admin/schedules`, `/admin/synthetic-backfill`, `/admin/ai-outcomes`, `/admin/status` | |
| AI | `/ai/entity-extraction`, `/ai/str-narrative`, `/ai/typology-suggestion`, `/ai/executive-briefing`, `/ai/alerts/{id}/explanation`, `/ai/cases/{id}/summary`, `/ai/outcomes/{dashboard,recent}` | |
| Stripe billing | `POST /webhooks/stripe` | Webhook receiver for subscription state. |
