...
**Kestrel · Enso Intelligence · 2026-05-06**
...
**Verifiable claims** — every persona-isolation assertion in §3 and §6 is backed by a unit test in `engine/tests/test_cross_bank.py` and an RLS policy verifiable via `pg_policies` on the production Supabase project. Procurement teams can request the live `pg_policies` dump and the production project reference under NDA.

**Document version**: 2026-05-06. Replaces all earlier informal descriptions of the cross-bank capability. Authoritative source for procurement, regulator review, and bank CTO due diligence.
